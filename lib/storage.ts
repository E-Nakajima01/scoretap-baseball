...
export const UNDO_STACK_KEY = "scoretap-baseball-undo-stack";
...
export function readUndoStack(): GameState[] {
  try {
    const raw = window.localStorage.getItem(UNDO_STACK_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as Partial<GameState>[];
    return Array.isArray(parsed) ? parsed.map(normalizeGame) : [];
  } catch {
...
export function writeUndoStack(stack: GameState[]) {
  window.localStorage.setItem(UNDO_STACK_KEY, JSON.stringify(stack.slice(0, MAX_UNDO_STACK)));
}
